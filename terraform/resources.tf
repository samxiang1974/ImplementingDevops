...
terraform {
  backend "s3" {
    bucket = "terraform4samx-remote-state-storage"
    key    = "terraform"
    region = "ap-southeast-2"
    encrypt = "true"
    dynamodb_table="terraform-statedb-samx"
  }
}
...
resource "aws_subnet" "private" {
  count=2
  vpc_id = "${aws_vpc.terraform-vpc.id}"
  cidr_block = "${cidrsubnet(var.vpc-cidr, 8, count.index + length(data.aws_availability_zones.all.names) + 1)}"
  availability_zone = "${element(data.aws_availability_zones.all.names, count.index)}"
...
git clone https://git-codecommit.ap-southeast-2.amazonaws.com/v1/repos/salt /srv/salt; chmod 700 /srv/salt
# Install SaltStack
